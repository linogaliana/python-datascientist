...
df_openfood = pd.read_csv(temp_dir, delimiter="\t",
...
def clean_labels_additional(data, yvar = 'product_name'):
    outvar = 'tokenized'
    data[outvar] = data[yvar].apply(lambda s: nltk.word_tokenize(s.lower()))
...
df_openfood = cleandata.clean_labels_additional(df_openfood)
```
...
wordcount_words(df_openfood, yvar = "libel_clean", type = "wordcount")
wordcount_words(df_openfood, "tokenized", type = "wordcount")
```
...
df_openfood_french = df_openfood[df_openfood['language'] == "fr"].dropna(subset = ["energy_100g"])
