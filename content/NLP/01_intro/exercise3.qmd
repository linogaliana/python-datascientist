...
::: {.content-visible when-profile="en"}
These preprocessing steps are starting to pay off since words with more
meaning are starting to emerge, notably the names of the characters
(Dantès, Danglars, etc.):
:::
...
#| label: fig-wordcloud-dumas-nostop-fr
...
::: {.content-visible when-profile="en"}
```{python}
#| fig-cap: "Word cloud produced from The Count of Monte Cristo after cleaning"
#| label: fig-wordcloud-dumas-nostop-en
plt.imshow(wordcloud_dumas_nostop, interpolation='bilinear')
plt.axis("off")
```
