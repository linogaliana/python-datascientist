...
1. Using the `wordCloud` function, create three word clouds to represent the most commonly used words by each author in the `horror` corpus[^random_state-en].
...
[^random_state-en]: To obtain the same results as shown below, you can set the argument `random_state=21`.
...
::: {.content-visible when-profile="en"}
```{python}
#| fig-cap: "Word cloud produced from The Count of Monte Cristo"
#| label: fig-wordcloud-dumas-en
plt.imshow(wordcloud_dumas, interpolation='bilinear')
plt.axis("off")
```
