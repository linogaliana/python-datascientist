...
pipeline_tfidf.fit(X_train)
```

## Trouver le texte le plus similaire

En premier lieu, on peut chercher le texte le plus proche, au sens de TF-IDF, d'une phrase donnée
...
Cela revient à effectuer une tâche de recherche d'information du type moteur de recherche. 

DETAILS

```{python}
X_train_tfidf = (
    pipeline_tfidf.transform(X_train)
)
...
# Get the top 4 similarities and their indices
...
details sur langchain

```{python}
from langchain_community.retrievers import TFIDFRetriever

# Convert to desired metadata structure
index_numbers = [
    {"id": i, "author": spooky_df["author"].iloc[i] } for i, _ in enumerate(X_train.index)
]

retriever = TFIDFRetriever.from_texts(
    X_train,
    metadatas = index_numbers
)
```
...
for best_echoes in retriever.invoke(text):
    documents += [{**best_echoes.metadata, **{"doc": best_echoes.page_content}}]
...
documents['score'] = cosine_similarity(
    pipeline_tfidf.transform(documents['doc']),
    pipeline_tfidf.transform([text])
)
```
