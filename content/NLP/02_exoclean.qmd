...
This method considers not only word frequencies but also which words follow others. It's particularly useful for disambiguating homonyms. The computation of _n-grams_ [^ngrams-en] is the simplest method for incorporating context.

[^ngrams-en]: We use the term _bigrams_ for two-word co-occurrences, _trigrams_ for three-word ones, etc.
