...
ample ce _package_. Les deux prochains chapitres, celui-ci et le
suivant, vont présenter quelques éléments structurants
de ce _package_. Ce chapitre prend la forme d'un tutoriel
...
carbones des Français[^empreinte]. 
...
aux secondes. Il ne s'agit pas, avec cet exercice, de construire une 
...
`Pandas` propose ainsi un schéma de données assez familier aux utilisateurs
...
les types lors de l'étape de _data cleaning_.
...
pouvez expérimenter dans les exercices :
...
dans le chapitre d'exercice (prochain chapitre).
...
L'affichage des _DataFrames_ dans les _notebooks_ est assez ergonomique.
...
## Dimensions et structure du _DataFrame_
...
Comme évoqué précédemment, `Pandas` propose énormément de méthodes utiles. 
...
valeurs présentes dans celle-ci. `Pandas` implémente
...
méthode `count` est disponible:

```{python}
df.count()
```

Cependant, cette dernière ne distingue pas les
valeurs dupliquées. Parfois, il est pratique
de connaître le nombre de valeurs uniques d'une
variable afin de savoir si elle dispose
de beaucoup de modalités ou non:

```{python}
df.nunique()
```

Les méthodes statistiques préimplementées vont
au-delà de simples sommes ou moyennes. Il est
possible de faire, par exemple, des calculs de quantiles
sur les différentes colonnes d'un jeu de données
...
Avec des données dont la distribution est non normalisée, celui-ci
peut être peu instructif. Le _log_ peut être une solution
...
L'objectif du [TP pandas](/python-datascientist/content/manipulation/02b_pandas_TP.qmd) est de se familiariser plus avec ces
