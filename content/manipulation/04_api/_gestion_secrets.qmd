...
:::: {.content-visible when-profile="en"}
## Understanding the principle with interactive documentation

To illustrate the use of authenticated APIs, we suggest exploring Insee's [API portal](https://api.insee.fr/catalogue/).

We will focus on the Sirene API but, on this portal, there are others, notably the Melodi API dedicated to retrieving a number of Insee _open data_ sources. The Sirene API is a queryable version of the [Sirene _open data_](https://www.insee.fr/fr/information/3591226) (in French).
