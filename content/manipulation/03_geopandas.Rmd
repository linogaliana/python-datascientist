...
C'est néanmoins possible d'aggréger à la fois les géométries et les 
...
Le système de projection est fondamental pour que la dimension spatiale soit bien interprétée par `python`. Si un système de projection est défini, il s'affiche dans la ligne `epsg (SRID)` lorsqu'on applique la fonction `head()` à un objet `sf`. Pour vérifier le système de projection d'une base de données, on peut utiliser `sf::st_crs`:
