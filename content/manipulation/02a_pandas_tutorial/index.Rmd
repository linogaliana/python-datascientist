...
Dans un script de production, lorsque l'on connait les types des variables du DataFrame,
...
`iloc` va se référer à l'indexation de 0 à `df.shape[0]` d'un
...
10     2014    31
```
Alors `df.loc[1, :]` donnera la première ligne de `df`tandis que
...
```{python}
df_new = df_new.drop(columns = ["Energie", "Agriculture"])
```
...
La syntaxe est quasiment transparente. On peut biensûr effecter des opérations
