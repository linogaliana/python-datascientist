...
try:
  ...
  response.raise_for_status()
  # Le serveur ne déclare pas de charset : `response.text` retomberait sur
  # l'ISO-8859-1 par défaut alors que le fichier est en UTF-8 (accents corrompus)
  cog_2023 = pd.read_csv(StringIO(response.content.decode("utf-8")))
...
filosofi = (
  ...
  .astype({c: "float" for c in ["POPULATION", "NIVVIE_MEDIAN", "TAUX_PAUVRETE"]})
)
