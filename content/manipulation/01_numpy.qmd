...
illustrons empiriquement quelques théorèmes incontournables de la statistique, à savoir: 
...
2. Pour chaque valeur $n$ de `grid`, calculer les grandeurs empiriques associées au préfixe $(X_1,\dots,X_n)$:
   1. la moyenne empirique 
      $$
      \bar X_n = \frac{1}{n}\sum_{i=1}^n X_i
      $$
   1. la variance empirique (avec diviseur $n$) :
      $$
      ...
      $$
3. Rappeler les valeurs théoriques de la moyenne $\mu$ et de la variance $\sigma^2$ de $\mathcal{U}\bigl([0,1]\bigr)$, puis comparer graphiquement les quantités empiriques à ces valeurs.
4. À quelle vitesse la moyenne empirique $\bar X_n$ converge-t-elle vers $\mu$ ? Proposer une illustration graphique (par exemple en traçant $|\bar X_n-\mu|$ en fonction de $n$ et en comparant à une décroissance en $1/\sqrt{n}$, ou en traçant $\sqrt{n}\,|\bar X_n-\mu|$).

:::

```{python}
# --- Correction Exercice 6, partie 1 : LGN sur U([0,1]) ---
import numpy as np
import matplotlib.pyplot as plt

# 1) Grille (dense au début + log ensuite)
def generate_grid(size_max=1000):
    n_small = np.arange(3, 200, 2)
    n_log = np.unique(np.round(np.logspace(np.log10(200), np.log10(size_max), 120)).astype(int))
    return np.unique(np.concatenate([n_small, n_log]))

# 2) Générateur flexible (fourni) + léger ajout: uniform par défaut sur [0,1]
def flexible_rng(
    dist="uniform",
    N_max=10000,
    seed=123,
    ...
    **kwargs
    ...
    # Valeurs par défaut pour l'uniforme: U([0,1])
    if d == "uniform":
        kwargs.setdefault("low", 0.0)
        kwargs.setdefault("high", 1.0)

    return rng_func(size=N_max, **kwargs)

# Paramètres
N_max = 100_000
grid = generate_grid(N_max)

# 3) Générer X ~ U([0,1]) de taille N_max
X = flexible_rng("uniform", N_max=N_max, seed=123)

# 4) Moments théoriques de U([0,1])
mu_theo = 0.5
var_theo = 1/12
...
# 6) Illustrations
# 6.1 Convergence de la moyenne vers mu
plt.figure()
plt.plot(grid, moments["mean"], label=r"$\bar X_n$")
plt.axhline(mu_theo, linestyle="--", label=r"$\mu=1/2$")
plt.xscale("log")
...
plt.axhline(var_theo, linestyle="--", label=r"$\sigma^2=1/12$")
plt.xscale("log")
