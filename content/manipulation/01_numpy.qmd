...
* La loi des grands nombres (@tip-exo2-fr);
...
`Numpy` is essential when running random simulations, which is very common in computational statistics with a family of methods known as Monte Carlo methods. The general principle is to replace a difficult theoretical calculation (an integral, a probability, an expectation) with a numerical approximation obtained by repeating a large number of random draws and averaging the quantity of interest, relying on the law of large numbers and the central limit theorem to quantify the accuracy of the estimate.

Let us empirically illustrate a few essential theorems of statistics through a series of exercises. This will allow us to explore: 

* The law of large numbers (@tip-exo2-en);
* The central limit theorem and its particular version in the case of coin tosses, the [de Moivre–Laplace theorem](https://en.wikipedia.org/wiki/De_Moivre%E2%80%93Laplace_theorem);
* Theoretical confidence intervals and their empirical counterpart through the _bootstrap_;
* The principle of Monte Carlo methods with the rejection algorithm.
...
We will need the following elements to initialise our data-generating process.
...
Together, these elements allow us to generate a random series of observations up to a size `N_max`, and then to observe the evolution of the empirical estimators of moments (mean and variance) when we only keep the first `n` observations, for different values of `n` given by `grid`. The goal is to illustrate the convergence of the empirical estimators to their theoretical values as `n` increases.

## The law of large numbers

We are used to making the fairly intuitive link between probability theory and statistics. This is made possible in particular thanks to theorems such as the [fundamental theorem of statistics (Glivenko–Cantelli theorem)](https://en.wikipedia.org/wiki/Glivenko%E2%80%93Cantelli_theorem). This relationship is the foundation of data science, and more broadly of statistics, in its inferential as well as descriptive dimension, since without formal mathematical intuitions we would struggle to generalise the interpretations drawn from observed data.

The [law of large numbers](https://en.wikipedia.org/wiki/Law_of_large_numbers) can be illustrated fairly easily through numerical simulations. We will simulate a repeated sequence of random draws, which will be the practical equivalent of the i.i.d. sequence $(X)_i$ of the theorem.
...
## Distribution of coin-toss results

::: {.callout-tip}
## Exercise 6, part 2. De Moivre–Laplace (binomial)

Consider an i.i.d. sequence $(X_i)_{i \ge 1}$ such that $X_i \sim \mathcal{B}(100,1/2)$ (number of heads in 100 tosses of a fair coin).

1. Plot the distribution of $X_1$ (probability mass function): $k \mapsto \mathbb{P}(X_1=k)$ for $k \in \{0,\dots,100\}$.
2. Compute the mean $\mu=\mathbb{E}[X_1]$ and the variance $\sigma^2=\mathrm{Var}(X_1)$. Give their numerical values.
3. For a value $N$ (for example $N \in \{50,200,1000,5000\}$), generate i.i.d. $X_1,\dots,X_N$ and compute $\bar X_N = \frac{1}{N}\sum_{i=1}^N X_i$.
4. By repeating the experiment a large number of times, plot the empirical distribution of
   $$
   Y_N = \sqrt{N}\,(\bar X_N-\mu).
   $$
5. On the same figure, superimpose the density of the normal distribution $\mathcal{N}(0,\sigma^2)$. In this case, you should get $\sigma^2=25$.

:::

::: {.callout-tip}
## Exercise 6, part 3. CLT for other distributions (finite variance)

Choose a distribution among: Poisson, exponential, uniform (your choice), specifying its parameters. Consider an i.i.d. sequence $(X_i)_{i \ge 1}$ following this distribution, with mean $\mu$ and variance $\sigma^2$ (both finite).

1. Give (or compute) $\mu$ and $\sigma^2$ for the chosen distribution.
2. For several values of $N$, by repeating the experiment a large number of times, plot the empirical distribution of
   $$
   Y_N = \sqrt{N}\,(\bar X_N-\mu)
   $$
   and compare it to the density of the normal distribution $\mathcal{N}(0,\sigma^2)$.
3. Optional: also study the standardised version
   $$
   Z_N = \sqrt{N}\,\frac{\bar X_N-\mu}{\sigma}
   $$
   and compare it to $\mathcal{N}(0,1)$.

:::

::: {.callout-tip}
## Exercise 6, part 4. Counterexample: the Cauchy distribution

Now consider an i.i.d. sequence $(X_i)_{i \ge 1}$ following a standard Cauchy distribution.

1. Generate a vector `X` of size `N_max` following a standard Cauchy distribution.
2. For each value $n$ of `grid`, compute the empirical mean $\bar X_n = \frac{1}{n}\sum_{i=1}^n X_i$ and plot $\bar X_n$ as a function of $n$.
3. Does the observed behaviour look like convergence to a constant? Compare qualitatively with part 1.
4. Do the "classical" LLN and CLT apply here? Justify by discussing the existence (or not) of $\mu$ and $\sigma^2$.
...
{{< include "01_numpy_exercises/_exo7_en.qmd" >}}
{{< include "01_numpy_exercises/_exo7_solution.qmd" >}}
