...
p_level + scale_x_continuous(limits = (3, 100))
```

Cela ressemble à une hyperbole du type $1/\sqrt{n}$. Vérifions si cela est cohérent en représentant $|\bar X_n - \mu|/\sqrt{n}$
...
Si on représente, au-delà des premières valeurs, le rapport entre l'erreur d'estimation et $\sqrt{n}$ (@fig-sqrtn-fr), on voit que l'intuition précédente était correcte. On a bien une erreur d'estimation qui oscille autour de $\sqrt{n}$. 

```{python}
#| fig-cap: "Représentation de l'erreur d'estimation et son lien avec le nombre d'observations"
#| label: fig-sqrtn-fr
p_level + scale_x_log10() + scale_y_log10()
```
