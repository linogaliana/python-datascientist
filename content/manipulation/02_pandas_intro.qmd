...
Pour rappel, afin d'exécuter les exemples de code dans un _notebook_ interactif, vous pouvez utiliser ces raccourcis selon votre environnement de prédilection:
...
![Cheasheet `Pandas`](https://cdn-images-1.medium.com/max/2000/1*YhTbz8b8Svi22wNVvqzneg.jpeg){width=60%}
...
d'utiliser la réassignation. Par exemple, pour créer une variable `dep` qui correspond aux deux premiers numéros du code commune (code Insee), 
...
```sql
SELECT * FROM df ORDER BY "Résidentiel"
```
...
```{python}
#| echo: true
df['NOM COMMUNE'].count()
df['NOM COMMUNE'].nunique()
```


En SQL, la première instruction serait `SELECT COUNT("NOM COMMUNE") FROM df`, la seconde `SELECT DISTINCT("NOM COMMUNE") FROM df`

La cohérence de la syntaxe `Pandas` permet de faire cela pour plusieurs colonnes de manière simultanée. En SQL cela serait possible mais commence à devenir assez verbeux:

```{python}
#| echo: true
df.loc[:, ['NOM COMMUNE', 'id insee']].count()
df.loc[:, ['NOM COMMUNE', 'id insee']].nunique()
```
...
`Pandas` nous renvoie un _DataFrame_ à plusieurs niveaux avec les informations voulues. 
...
dans un DataFrame avec `dropna()` avec le paramètre `subset`.

```{python}
ventes.dropna(subset=["produit"])
```

On ne perd plus qu'une ligne.
