...
::: {#fig-convergence-mean-fr layout-ncol=2}

```{python}
#| label: fig-convergence-mean-fr-normal
#| fig-cap: "Echelle normale"
p.show()
```

```{python}
#| label: fig-convergence-mean-fr-log
#| fig-cap: "Echelle logarithmique"
p_log.show()
```

Convergence de la moyenne vers l'espérance théorique (question 3, @tip-exo2-fr)
:::
...
::: {#fig-convergence-mean-en layout-ncol=2}

```{python}
#| label: fig-convergence-mean-en-normal
#| fig-cap: "Linear scale"
p.show()
```

```{python}
#| label: fig-convergence-mean-en-log
#| fig-cap: "Logarithmic scale"
p_log.show()
```

Convergence of the mean to the theoretical expectation (Question 3, @tip-exo2-en)
:::
...
::: {.content-visible when-profile="en"}
With @fig-convergence-var-en we can see that the intuitive version of the empirical variance, that is, the one with $n$ in the denominator, systematically tends to underestimate the variance. It is indeed a biased estimator, even though the bias tends to 0 as $n$ grows. The sample variance, the one with $n-1$, follows broadly the same dynamics but gets closer to the theoretical value sooner.
:::
...
::: {#fig-convergence-var-fr layout-ncol=2}

```{python}
#| label: fig-convergence-var-fr-normal
#| fig-cap: "Echelle normale"
#| message: false
p_lev_labeled.show()
```
...
```{python}
#| label: fig-convergence-var-fr-zoom
#| fig-cap: "Zoom sur les premières observations"
#| message: false
p_zoom_labeled.show()
```

Convergence de la variance empirique vers la variance théorique (question 3, @tip-exo2-fr)
:::
...
::: {#fig-convergence-var-en layout-ncol=2}

```{python}
#| label: fig-convergence-var-en-normal
#| fig-cap: "Linear scale"
#| message: false
p_lev_labeled.show()
```
...
```{python}
#| label: fig-convergence-var-en-zoom
#| fig-cap: "Zoom on the first observations"
#| message: false
p_zoom_labeled.show()
```

Convergence of the empirical variance to the theoretical variance (Question 3, @tip-exo2-en)
:::
