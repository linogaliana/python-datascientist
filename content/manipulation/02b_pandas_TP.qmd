...
plutôt qu'un chemin absolu même si nous préconiserons ensuite de privilégier directement la lecture depuis internet lorsqu'elle est possible. 
...
## Importer et découvrir des données depuis un CSV
...
De manière automatique un _dataframe_ `Pandas`
est mis en forme lorsqu'il est visualisé depuis un _notebook_.
...
Heureusement, le tout jeune _package_ [`great_tables`](https://posit-dev.github.io/great-tables/get-started/) permet, de manière programmatique
et assez simple, la création de tableaux
qui n'ont rien à envier à des productions manuelles faites dans `Excel`
et difficilement répliquables. `great_tables` construit des tableaux
_html_ ce qui offre une grande richesse dans la mise en forme. 
