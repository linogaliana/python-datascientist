...
la première base est relativement complète, la seconde comporte beaucoup de valeurs manquantes liée à la gestion du secret statistique.
...
print(100*df_big_city['probleme'].mean()) #8,33 %
```
...
#df.reset_index().groupby('dep').sum(numeric_only = True).sum(axis = 1, numeric_only = True).head() #version simplifiee ? Non car dep doit être un index pour la suite des questions
