...
(
    GT(emissions_table)
    .tab_header(title=md("**Empreinte carbone**"), subtitle = "Premières statistiques à affiner")
    .fmt_number("empreinte", decimals = 2, scale_by=0.1)
    .fmt_number("population", compact=True)
    .fmt_integer("revenu", scale_by = 1/1000)
    .fmt_percent(cs.starts_with("raw_perc"), decimals = 2)
    .cols_move(
    ...
    .tab_spanner(label="Empreinte", columns=cs.ends_with("empreinte"))
    .tab_spanner(label="Revenu médian", columns=cs.ends_with("revenu"))
    .tab_spanner(label="Population", columns=cs.ends_with("population"))
    .tab_source_note(
        source_note=md("*Source: Données de l'Ademe*")
    )
)
```
