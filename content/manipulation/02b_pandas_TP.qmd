...
<!-- KA : je ne comprends pas : Tu ne parles plus de Pynsee mais tu l'utilises plus bas.  -->
...
<!-- KA : Attention ! sans cette ligne la fin du TP ne marche pas chez moi car il ne veut pas faire de somme de variables objects. Je te laisse le soin de la reformuler au format bonnes pratiques si tu le souhaites.  -->

```{python}
filosofi[filosofi.columns[2:len(filosofi.columns)]] = filosofi[filosofi.columns[2:len(filosofi.columns)]].apply(pd.to_numeric)
```
...
```{=html}
<div class="alert alert-success" role="alert">
<h3 class="alert-heading"><i class="fa-solid fa-pencil"></i> Exercice 3: structure des données</h3>
```
...
5. Déterminer la taille moyenne (variable nombre de personnes: `NBPERSMENFISC16`) et quelques statistiques descriptives de ces données.
Comparer aux mêmes statistiques sur les données où libellés et codes communes coïncident.

6. Vérifier les grandes villes (plus de 100 000 personnes),
la proportion de villes pour lesquelles un même nom est associé à différents codes commune.

7. Vérifier dans `filosofi` les villes dont le libellé est égal à Montreuil.
...
x = filosofi.groupby('LIBGEO').count()['CODGEO']
x = x[x>1]
x = x.reset_index()
x
```
...
#| output: false
# Question 3
filosofi[filosofi['LIBGEO'].isin(x['LIBGEO'])]
```

```{python}
#| output: false
# Question 4
filosofi[filosofi['LIBGEO'].isin(x['LIBGEO'])].sort_values('LIBGEO')
```

```{python}
#| output: false
# Question 5
print(filosofi[filosofi['LIBGEO'].isin(x['LIBGEO'])]['NBPERSMENFISC16'].describe())
print(filosofi[~filosofi['LIBGEO'].isin(x['LIBGEO'])]['NBPERSMENFISC16'].describe())
```
...
#| output: false
# Question 6
emissions_big_city = filosofi[filosofi['NBPERSMENFISC16']>100000].copy()
emissions_big_city['probleme'] = emissions_big_city['LIBGEO'].isin(x['LIBGEO'])
...
emissions_merged = emissions.merge(filosofi, how = "left", left_index = True, right_index = True)

print(emissions_merged[emissions_merged['LIBGEO'].isna()]['emissions'].mean())
print(emissions_merged[~emissions_merged['LIBGEO'].isna()]['emissions'].mean())
```
...
emissions_merged = emissions.merge(filosofi, left_index = True, right_index = True)
