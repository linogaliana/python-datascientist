...
ciqual['nom_clean'] = ciqual['alim_nom_fr'].apply(lambda s: unidecode(s))
ciqual.head(2)
```
...
```{python}
#| echo: false
ciqual['nom_clean'] = ciqual['nom_clean'].str.upper()
ciqual.head(2)
```
...
ciqual['nom_clean'] = ciqual['nom_clean'].str.replace(
...
```{python}
#| echo: false
ciqual["nom_clean"] = ciqual["nom_clean"].str.replace(r'[^a-zA-Z]', ' ', regex=True)
ciqual.head(2)
```
...
```{python}
#| echo: false
ciqual["nom_clean"] = ciqual["nom_clean"].str.replace(r'([ ]{2,})', ' ', regex=True)
ciqual.head(2)
```
