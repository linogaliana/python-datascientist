...
En l'occurrence, on va utiliser les données Filosofi (données de revenus) au niveau communal, dans la 
...
pouvez utiliser la fonction `pynsee.get_file_list()`. 
...
Ici, `meta['label'].str.contains(r`"Filosofi.*2016"`)` signifie:
 _"`pandas` trouve moi tous les labels où sont contenus les termes Filosofi et 2016_"
 (`.*` signifiant _"peu m'importe le nombre de mots ou caractères entre"_)
