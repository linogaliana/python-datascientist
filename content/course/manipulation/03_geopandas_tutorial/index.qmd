...
communes = communes.rename({"insee_com": 'insee'}, axis = 'columns')
...
```{python}
communes.groupby('dep').sum().sort_values('surface', ascending = False)
```

ou les plus grandes communes de France métropolitaine :

```{python}
communes[communes.dep != "97"].sort_values('surface', ascending = False)
```
...
```{python}
fig,ax = plt.subplots(figsize=(10, 10))
communes[communes.dep != "97"].dissolve(by='dep', aggfunc='sum').plot(ax = ax, column = "surface")
ax.set_axis_off()
ax
```
