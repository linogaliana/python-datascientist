...
```{python}
ax = region.loc[regions['INSEE_DEP']=="75"].plot()
ax.set_axis_off()
```
...
```{python}
arrondissements = cartiflette.s3.download_vectorfile_s3_all(
    values = "75",
...
```{python}
france.groupby('INSEE_DEP').sum().sort_values('surface', ascending = False)
```
...
Dans les cartes qui reposent sur cette projection, le Groënland paraît avoir
la même surface que l'Amérique du Sud. Pourtant, cette dernière est 8 fois 
plus grande. 
Il existe de nombreuses représentations possibles du monde, plus ou moins 
alambiquées. Par exemple,
