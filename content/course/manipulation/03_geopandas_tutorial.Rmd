...
On peut se rassurer en affichant les données sur la carte des communes d'Île-de-France. Découvrez ci-dessous par étape les différentes lignes de commandes permettant d'afficher une carte complète : 
...
2. Y ajouter en dessous les contours des communes
...
3. Y ajouter un fond de carte de type OSM
...
4. Retirer les axes des coordonnées
...
Lors des étapes d'agrégation, `groupby` ne conserve pas les géométries. 
...
:warning: Les données sont en système de coordonnées WGS 84 ou pseudo-Mercator (epsg: `4326`) et ne sont pas projetées. C'est un format approprié lorsqu'il s'agit d'utiliser un fonds
...
Outre la représentation graphique simplifiée, sur laquelle nous reviendrons ultérieurement, l'intérêt principal d'utiliser
...
Une méthode qu'on utilise régulièrement est `centroid` qui, comme son nom l'indique, recherche le centroïde de chaque polygone et transforme ainsi des données surfaciques en données ponctuelles. Par exemple, pour
