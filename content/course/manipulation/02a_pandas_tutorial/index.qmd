...
Par exemple, avec le `pandas.DataFrame` `df`:


```{python}
data = {'year': [2012, 2014, 2013, 2014], 'sale': [55, 40, 84, 31]}
df = pd.DataFrame(data, index=[1, 4, 7, 10])
df
```

- `df.loc[1, :]` donnera la première ligne de `df` (ligne où l'indice `month` est égal à 1) ;
- `df.iloc[1, :]` donnera la deuxième ligne (puisque l'indexation en `Python` commence à 0) ;
- `df.iloc[:, 1]` donnera la deuxième colonne, suivant le même principe.
...
::: {.cell .markdown}
```{=html}
<details><summary><code>Téléchargement de <code>pynsee</code> avec barre de progrès 👇</summary>
```
