...
## Requêter une API de l'Ademe pour obtenir des diagnostics energétiques
...
sur le site [API GOUV](https://api.gouv.fr/documentation/api_dpe_logements).
...
On peut utiliser `requests` et `pandas` pour récupérer les données:
...
code_commune="01450"
size = 100
url2 = f"https://koumoul.com/data-fair/api/v1/datasets/dpe-france/lines?page=1&after=10&format=json&q_mode=simple&qs=code_insee_commune_actualise%3A%22{code_commune}%22&size={size}&select=%2A&sampling=neighbors"
