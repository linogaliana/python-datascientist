...
Le projet requiert l'installation de dask 
```
...
Plusieurs points sont mis en avant et son résumés ci-dessous: 
- `dask` ressemble fortement en termes de syntaxe à `pandas`, `numpy`
- `dask` peut être utilisé sur un ordinateur seul ou sur un cloud cluster. Peut traiter des bases de 100giga sur un ordinateur portable, voire 1TB sur un ordi fixe (sans même avoir besoin d'un cluster, et avec les machines appropriées)
- `dask` utilisé sur une machine seule (et non sur un cloud cluster) requiert peu de temps d'installation (fait partie de la suite `Anaconda`)

## Comment `dask` se compare à `Spark` ?
- `Spark` est écrit en Scala, avec possibilité d'écrire notamment en `python` (via `Pyspark`). Il permet d'interagir avec du JVM
 alors que `dask` est écrit en Python, et fonctionne bien avec Python. Il permet d'interagir avec du code C/C++ entre autres
...
- `Spark` permet de très bien faire des opérations classiques SQL et des ETLs, alors que `dask` permet de bien interagir avec `scikit-learn` et de faire de la modélisation. Pour faire du ML avec `Spark` il faut aller piocher dans `Spark MLLib`
**Globalement, il faut retenir que `dask` comme `spark` ne sont intéressants que pour des données dont le traitement engendre des problèmes de RAM**.

## Démonstration de quelques features de `dask`

Nous allons utiliser les données DVF pour montrer quelques éléments clefs de `dask`.
