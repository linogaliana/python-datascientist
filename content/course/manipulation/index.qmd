...
du _data scientist_. La modélisation, pour être pertinente,
nécessite en effet un certain recul sur les données ; il n'est
pas possible de faire l'impasse sur celle-ci.

Ce cours, 
comme de nombreuses ressources introductives sur
la _data science_ [@wickham2023r, @vanderplas2016python ou @mckinney2012python],
...
Néanmoins, il s'agit d'un objet qui, en `Python`, ne s'est
imposé que récemment, notamment grâce au package `Pandas` créé
par [Wes McKinney](https://fr.wikipedia.org/wiki/Wes_McKinney). 
Ce tableau bi-dimensionnel, permettant de stocker dans différentes
variables des informations structurées, est un objet pratique à
manipuler au quotidien. 
La logique SQL, un langage qui a déjà plus de cinquante ans, 
a infusé dans les esprits et la plupart des librairies
de manipulation de données s'inspirent de cette logique.
...
L'écosystème `pandas` représente un couteau-suisse
