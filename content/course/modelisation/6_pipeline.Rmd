...
```{r, echo = FALSE}
knitr::include_graphics('correlation_matrix_dvf.png')
```
...
```{python fit randomforest, results = "hide"}
...
```{python, echo = FALSE, include = FALSE}
plt.savefig("varimportance2.png")
```
...
```{r, echo = FALSE}
knitr::include_graphics('predict1.png')
```
...
des hyperparamètres amenant à la meilleure prédiction
...
param_grid = {
    "randomforest__n_estimators": np.linspace(10,200, 50).astype(int),
    "randomforest__max_depth": np.linspace(2,10,2).astype(int)
}
grid_search = GridSearchCV(pipe, param_grid=param_grid)
grid_search.fit(pd.concat((X_train, X_test), axis=0), np.concatenate([y_train,y_test]))
```

Si vous exécutez la cellule ci-dessus, cela risque de donner de nombreux
warnings. En effet, on est face à un problème de *data leaking* car l'échantillon
n'est pas balancé entre nos classes (les départements). Certaines classes se 
retrouvent hors de l'échantillon d'estimation mais dans l'échantillon de prédiction.

Pour éviter cela, il serait mieux de définir explicitement le schéma de
validation croisée à mettre en oeuvre. Cela pourrait être fait avec
