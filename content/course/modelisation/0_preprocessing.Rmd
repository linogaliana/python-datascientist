...
mais l'exercice 1 permet, à ceux qui le désirent, d'essayer de reproduire la constitution de la base de données. 
...
```{python, warning = FALSE, echo=TRUE, message = FALSE, include = FALSE}
import numpy as np
...
```{python, warning = FALSE, echo=FALSE, message = FALSE, include = FALSE}
...
fig.write_json("people_vote.json")
...
La première étape nécessaire à suivre avant de modéliser est de déterminer les variables à inclure dans le modèle.
...
1. Créer un DataFrame `df2` plus petit avec les variables `winner` et `unemployment`, `median_age`, `asian`, `black`, `white_not_latino_population`,`latino_population`, `gini_coefficient`, `less_than_high_school`, `adult_obesity`, `median_earnings_2010_dollars`
...
```{python scatter_matrix, warning = FALSE, message = FALSE}
#| include: false
#| echo: false
...
souvent nécessaire d'appliquer des tâches de *preprocessing*, c'est-à-dire des tâches de modification de la distribution des données pour les rendre
...
```{python standardisation, message = FALSE, warning = FALSE}
#| include: false
#| echo: false
