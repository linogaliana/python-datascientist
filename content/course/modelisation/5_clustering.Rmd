...
* au sein de chaque classe, les observations sont homogènes (variance intra-classe minimale)
...
Dans certaines bases de données, on peut se retrouver avec quelques examples labellisés mais la plupart
...
de manière intelligente afin de ne pas être trop proches
...
aux observations du cluster le plus proche est inférieure à la moyenne des distance de l'observation aux
...
moin `min_samples` voisins, alors l'observation sera une *core instance*.

Les observations qui ne sont pas des core instance et qui n'en ont pas dans leur voisinage selon une distance espilon
vont être détectées comme des anomalies.


### Les mélanges de gaussiennes

Pour aller plus loin, voir le cours Probabilités numériques et statistiques computationnelles, M1 Jussieu, V.Lemaire et T.Rebafka :
http://www.proba.jussieu.fr/pageperso/rebafka/#enseignement
