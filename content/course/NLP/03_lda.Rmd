...
corpus_lda = lda.components_[0]
first_topic_words = [tf_feature_names[l] for l in corpus_lda.argsort()[:-50-1:-1]]

fig, axs = plt.subplots(len(lda.components_) // 2 + 1, 2)

for i in range(len(lda.components_)):
    k = i // 2
    j = (i - k*2)
    wordcloud = WordCloud(stopwords=stop_words, background_color="black",width = 2500, height = 1800)
    wordcloud = firstcloud.generate(" ".join(first_topic_words))
    #plt.title("Wordcloud pour le sujet {}".format(i))
    axs[k][j].axis('off')
