...
{{% panel status="exercise" title="Exercise: les noms de produits dans l'openfood database" icon="fas fa-pencil-alt" %}}
L'objectif de cet exercice est d'explorer les termes les plus fréquents
dans les noms de produits de l'openfood database
{{% /panel %}}
...
os.system("python spacy download fr_core_news_sm")
```

Dans un notebook jupyter, il se peut qu'il soit nécessaire de relancer le kernel

2. Importer les données de l'openfood database à partir du code suivant
...
1. Appliquer le modèle téléchargé précedemment pour déterminer le langage
2. Ne récupérer que les libellés français
...
d'entités nommées. Cela vous semble-t-il satisfaisant ?
