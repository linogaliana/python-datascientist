...
# Réaliser une animation avec `Python`

Téléchargement fichier décès
...
```{python}
n_death = deces.loc[deces["prenom"] == "JEAN"].groupby('annee').agg({"nomprenom": "size"}).reset_index()
n_death.rename({"nomprenom": "nombre"}, axis = "columns", inplace = True)
naissance = naissance.loc[naissance["preusuel"] == "JEAN"].loc[naissance["annais"] != "XXXX"]
naissance.rename({"annais": "annee"}, axis = "columns", inplace = True)
n_death["annee"] = n_death["annee"].astype(int)
naissance["annee"] = naissance["annee"].astype(int)
```

```{python}
table_complete = pd.concat([
  n_death.assign(source = "death"),
  naissance.assign(source = "birth")
])
table_complete = table_complete.loc[table_complete["annee"]>1970]
```
...
def animate(i):
    table_complete = df.iloc[:int(i+1)] 
    graph = sns.pointplot(x="nombre", y="annee", hue="source", data=data, markers=["o", "x"], linestyles=["-", "--"])
    return graph

ani = matplotlib.animation.FuncAnimation(fig, animate, frames=len(table_complete.index),interval=700,repeat=True)
plt.show()
```
