...
1. Importer les données de compteurs de vélos. Vous pouvez utiliser l'url <https://minio.lab.sspcloud.fr/projet-formation/diffusion/python-datascientist/bike.csv>. ⚠️ Il s'agit de données
...
import pandas as pd
df = pd.read_csv("https://minio.lab.sspcloud.fr/projet-formation/diffusion/python-datascientist/bike.csv", compression = 'gzip')
```
...
Les valeurs chiffrées des graphiques seront différentes de celles de la page en ligne, c'est normal, nous travaillons sur des données plus anciennes. 
...
A ce stade, la figure 1 devrait avoir l'aspect suivant:
...
```{python}
g.figure.get_figure().savefig('featured_matplotlib.png')
```
...
```{=html}
<div class="alert alert-success" role="alert">
<h3 class="alert-heading"><i class="fa-solid fa-pencil"></i> Exercice 3 : Refaire les graphiques</h3>
```
...
```{=html}
<div class="alert alert-success" role="alert">
<h3 class="alert-heading"><i class="fa-solid fa-pencil"></i> Exercice 4 : Premier graphique avec plotly</h3>
```
