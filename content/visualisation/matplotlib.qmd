...
description-en: |
    An essential part of the _data scientist's_ job is to be able to synthesize information into powerful graphical representations. This chapter looks at the challenges of data representation with `Python`, the ecosystem for doing this. It also opens the door to interactive data representation with `Plotly` or `Altair`.
...
#| echo: false
#| output: false
if os.getenv('QUARTO_PROFILE') == "fr":
    title="Les 10 compteurs avec la moyenne horaire la plus élevée"
    xaxis="Nom du compteur"
    yaxis="Moyenne horaire"
```
...
::: {.content-visible when-profile="en"}
# A first figure with `Pandas`' `Matplotlib` API
...
{{< include "01_matplotlib/_plotnine.qmd" >}}
...
{{< include "01_matplotlib/_lollipop.qmd" >}}
...
```{python}
#| echo: false
columns_mapping_fr = columns_mapping
```
...
#| echo: false
if os.getenv("QUARTO_PROFILE") == "fr":
    columns_mapping = columns_mapping_fr 
```
...
#| echo: false
#| label: tbl-gt
#| tbl-cap: Visualisation alternative sous forme de table
...
::: {.content-visible when-profile="fr"}
# Des graphiques réactifs grâce aux interfaces avec `Javascript`
...
# Reactive charts with `Javascript` wrappers
...
2. Modifier le texte apparaissant en surbrillance (_hover text_). 
...
fig.update_traces(
    hovertemplate="<b>%{y}</b><br>Moyenne horaire: %{x:.1f}<extra></extra>"
)
```
...
fig_dark.update_layout(barmode='stack', yaxis={'categoryorder':'total ascending'})

fig_dark.update_traces(
    hovertemplate="<b>%{y}</b><br>Moyenne horaire: %{x:.1f}<extra></extra>"
)
```

```{python}
#| label: fig-plotly-double
#| layout-ncol: 1
fig.show()
fig_dark.show()
```
