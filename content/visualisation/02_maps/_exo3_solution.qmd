...
[^landes-epsg-en]: This calculation is possible because both datasets are in the Lambert 93 projection, which allows for geometric operations (including surface area calculations).
...
```{python}
#| echo: true
f"Part du couvert forestier dans les Landes: {float(foret.area.sum()/landes.area.sum()):.0%}"
```
