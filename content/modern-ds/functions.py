...
def import_openfood_s3(cols=openfoodcols, nrows=None):
    ...
    fs.download("projet-relevanc/diffusion/openfood.csv", "openfood.csv")

    cols = pd.Series(cols).str.replace("_score", "_grade").tolist()

    data = pd.read_csv(
        filepath_or_buffer="openfood.csv", usecols=cols, nrows=nrows, encoding="utf-8"
    )
    data = data.dropna()
...
def import_openfood(from_latest=False, cols=openfoodcols, nrows=None):
    ...
    data = pd.read_csv(
        filepath_or_buffer="openfood.csv",
        delimiter="\t",
        usecols=cols,
        nrows=nrows,
        encoding="utf-8",
...
def import_ciqual():
    ...
    if os.path.isfile("ciqual.csv") is False:
        ciqual = pd.read_excel(
            path,
            usecols=["alim_nom_fr", "Energie, Règlement UE N° 1169/2011 (kcal/100 g)"],
        )
        ciqual = ciqual[
            ciqual["Energie, Règlement UE N° 1169/2011 (kcal/100 g)"] != "-"
        ]
...
def match_product(liste_produits, dataset_comparaison, yvar_comparaison="alim_nom_fr"):
    ...
    out = [
        rapidfuzz.process.extract(
            c,
            dataset_comparaison[yvar_comparaison].tolist(),
            scorer=rapidfuzz.fuzz.token_sort_ratio,
            limit=1,
        )
        for c in liste_produits
    ]
    out = [item for sublist in out for item in sublist]
    dist_leven = pd.DataFrame(
        [c for c in out], columns=["best_match", "distance", "index_ciqual"]
    )
...
def index_elastic(es, index_name: str, setting_file: str, df=pd.DataFrame):
    ...
    deque(
        parallel_bulk(
            client=es,
            actions=gen_dict_from_pandas(index_name, df),
            chunk_size=2000,
            thread_count=4,
        )
    )
