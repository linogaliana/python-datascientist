...
Avec ce modèle, on se trompe ainsi, en moyenne de `r round(py$rmse, 1)` points
et on explique `r round(py$rsq*100)` % de la variance observée des scores républicains.
Le nuage de point suggère 

```{python}
# Question 4
sns.scatterplot(x=y_pred, y=np.sqrt((y_test - y_pred)**2))
```
