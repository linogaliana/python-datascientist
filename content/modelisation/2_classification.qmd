...
Créer un arbre de décision avec les arguments par défaut, l'entraîner. Visualiser. Quel est le problème ? 
Evaluer sa performance prédictive. Comparer à celle d'un arbre dont la profondeur est 4. Conclure.
Représenter cet arbre de 4 avec la méthode gridviz
Regarder les mesures de performance suivante : `accuracy`, `f1`, `recall` et `precision`.
...
clf_depth4 = tree.DecisionTreeClassifier(max_depth=4)
clf_depth4.fit(X_train, y_train)

clf_depth4.score(X_test, y_test)
...
dot_data = tree.export_graphviz(clf_depth4, out_file=None,
