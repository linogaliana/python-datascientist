...
```{python, echo = TRUE}
df2 = votes.loc[:,~votes.columns.str.endswith(('_democrat','_green','_other'))]
```
...
2. Estimer un modèle LASSO. Afficher les valeurs des coefficients: qu'en déduire sur le modèle idéal ? Quelle variable a une valeur non nulle ?
...
lcv = LassoCV(alphas=my_alphas ,normalize=False,fit_intercept=False,random_state=0,cv=5).fit(np.delete(df3, yindex, axis = 1), df3[:,yindex])
lcv.alpha_
```
