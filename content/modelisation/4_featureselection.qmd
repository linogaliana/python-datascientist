...
3. En vous inspirant de [cet exemple](https://scikit-learn.org/stable/modules/generated/sklearn.pipeline.Pipeline.html), créer un _pipeline_ effectuant les étapes suivantes:
    + Impute les valeurs manquantes à la moyenne avec `SimpleImputer()`
    + Standardise les *features* (c'est-à-dire les variables autres que la variable `per_gop`) avec `StandardScaler`
    + Estime un LASSO pénalisé
...
#3. Garder uniquement les variables numériques et standardiser.
preprocessor_step1 = ColumnTransformer([
  ('imputer', SimpleImputer(), X_train.columns.tolist())
  ])
...
pipeline = Pipeline(
    steps = [
      ("Imputation", preprocessor_step1),
      ("Standardisation", preprocessor_step2),
