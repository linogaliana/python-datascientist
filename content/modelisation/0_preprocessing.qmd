...
c'est aussi prendre le risque de retirer un nombre important de lignes, et avec cela de l'information importante pour un modèle.
...
- Imputation par régression : se servir d'autres variables pour essayer d'interrpoler une valeur de remplacement adaptée
...
Le package `scikit` permet de faire de l'imputation de manière très simple.

### Gestion des outliers

Dans de nombreuses bases de données, il n'est pas rare de trouver des observations ou valeurs aberrantes, appelées **outliers**.
...
Plusieurs packages permettent de faire ce type d'opérations, qui sont parfois plus complexes si on s'intéresse aux outlier sur des n-uplets de variables.
...
Pour faire vos premiers pas en modélisation, notamment sur le preprocessing de données, vous pouvez également consulter le sujet 3 du Funathon 2023, *Explorer les habitudes alimentaires de nos compatriotes*, sur le [SSP Cloud](https://www.sspcloud.fr/formation?search=funath&path=%5B%22Funathon%202023%22%5D) ou sur [Github](https://github.com/InseeFrLab/funathon2023_sujet3/).
...
- Prédiction de l'IMC : Premiers pas vers les méthodes de ML supervisé et les preprocessings associés
