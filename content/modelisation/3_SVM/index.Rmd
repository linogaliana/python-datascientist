...
Pour illustrer le travail de données nécessaire pour construire un modèle de Machine Learning, mais aussi nécessaire pour l'exploration de données avant de faire une régression linéaire, nous allons partir du jeu de données déja
présenté sur le [résultat des élections US 2016 au niveau des comtés](#preprocessing)
...
5. Changer de variables *x*. Prendre uniquement `share_2016_republican` et `Median_Household_Income_2019`. Regarder les résultats, notamment la matrice de confusion
...
C'est pour cette raison que l'étape de sélection de variable est cruciale. En utilisant uniquement le résultat passé du vote démocrate et le revenu (`share_2016_republican` et `Median_Household_Income_2019`), on obtient un résultat beaucoup plus cohérent:
