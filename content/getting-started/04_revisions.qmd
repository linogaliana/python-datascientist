...
function renderTeaser(doc) {
  ...
  const authorsHTML = authors
   ...
   const name = (lang == "en") ? name.en : name.fr 
   const deploymentUrl = (lang == "en") ? deploymentUrl.en : deploymentUrl.fr
   const abstract = (lang == "en") ? abstract.en : abstract.fr


  return `
    <div class="gridcard-item">
      <div class="gridcard-body">
        <a class="gridcard-title" href="${deploymentUrl}">${name}</a>
        ${abstract ? `<p class="gridcard-description">${abstract}</p>` : ''}
        <a class="btn btn-outline-primary btn-launch" href="${deploymentUrl}" target="_blank">${textButton}</a>
      </div>
    </div>
  `;
}
