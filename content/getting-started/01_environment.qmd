...
content_html = '''
    ...
    <div class="callout callout-tip">
        <div class="callout-header-tip">
            <i class="fa-solid fa-lightbulb"></i> Objet de ce chapitre
        </div>
        <div class="callout-body">
            <ul>
<li>Exposer les enjeux liés à la modularité de <code>Python</code> ;</li>
<li>Introduire à l\'environnement quotidien de travail des <em>data scientists</em> à savoir le <em>notebook</em> ;</li>
<li>Illustrer l\'utilisation des <em>notebooks</em> dans le cadre de ce cours ;</li>
<li>Présenter la démarche à adopter quand on rencontre une erreur ;</li>
<li>Comprendre les enjeux d\'une formation continue en <code>Python</code> et découvrir quelques bonnes ressources pour cela.</li>
</ul>
        </div>
    </div>
...
content_html = '''
    ...
    <div class="callout callout-tip">
        <div class="callout-header-tip">
            <i class="fa-solid fa-lightbulb"></i> Objective of This Chapter
        </div>
        <div class="callout-body">
            <ul>
<li>Expose the issues related to the modularity of <code>Python</code>;</li>
<li>Introduce the daily working environment of <em>data scientists</em>, namely the <em>notebook</em>;</li>
<li>Illustrate the use of <em>notebooks</em> within the framework of this course;</li>
<li>Present the approach to take when encountering an error;</li>
<li>Understand the importance of ongoing training in <code>Python</code> and discover some good resources for it.</li>
</ul>
        </div>
    </div>
...
content_html = '''
    ...
    <div class="callout callout-note">
        <div class="callout-header-note">
            <i class="fa-solid fa-comment"></i> Illustration
        </div>
        <div class="callout-body">
            <p>Si vous avez accès à une ligne de commande dans un environnement où <code>Python</code> est disponible et bien configuré (notamment ajouté au <a href="https://fr.wikipedia.org/wiki/Variable_d%27environnement#%3CPATH%3E_pour_l\'emplacement_des_ex%C3%A9cutables"><code>PATH</code></a>), vous pouvez déjà faire du <code>Python</code> par le biais de la ligne de commande[^vscode]</p>
<p>~~~python
python --version
python -c "print(3+3)"
echo "print(3+3)" &gt; example.py &amp;&amp; python example.py &amp;&amp; rm example.py #&lt;1&gt;
~~~
1. Un exemple d\'utilisation de création de fichier <code>example.py</code> depuis la ligne de commande et d\'utilisation de celui-ci par la ligne de commande. Créer des fichiers depuis la ligne de commande n\'étant pas un mode de travail réaliste, nous aurons l\'occasion d\'expliquer comment créer des fichiers par le biais d\'un éditeur de texte.</p>
        </div>
    </div>
...
content_html = '''
    ...
    <div class="callout callout-note">
        ...
        <div class="callout-body">
            <p>If you have access to a command line in an environment where <code>Python</code> is available and properly configured (including being added to the <a href="https://en.wikipedia.org/wiki/Environment_variable#%3CPATH%3E_for_executable_locations"><code>PATH</code></a>), you can already run <code>Python</code> through the command line[^vscode-en]</p>
<p>~~~python
python --version
python -c "print(3+3)"
echo "print(3+3)" &gt; example.py &amp;&amp; python example.py &amp;&amp; rm example.py #&lt;1&gt;
~~~
1. An example of creating a file <code>example.py</code> from the command line and using it from the command line. Since creating files from the command line is not a realistic working mode, we will explain how to create files using a text editor.</p>
        </div>
    </div>
...
content_html = '''
    ...
    <div class="callout callout-note">
        ...
        <div class="callout-body">
            <p>Voici une illustration de la manière dont fonctionne la gestion de <em>packages</em> en <code>Python</code></p>
<p>~~~python
python -c "import geopandas as gpd" #&lt;1&gt;
pip install geopandas
python -c "import geopandas as gpd ; print(gpd.GeoDataFrame)" #&lt;2&gt;
~~~
1. Cela devrait provoquer une erreur si <code>GeoPandas</code> n\'est pas installé
<code>ModuleNotFoundError: No module named \'geopandas\'</code>
2. L\'erreur devrait avoir disparue</p>
        </div>
    </div>
...
content_html = '''
    ...
    <div class="callout callout-note">
        ...
        <div class="callout-body">
            <p>Here is an illustration of how package management works in <code>Python</code></p>
<p>~~~python
python -c "import geopandas as gpd" #&lt;1&gt;
pip install geopandas
python -c "import geopandas as gpd ; print(gpd.GeoDataFrame)" #&lt;2&gt;
~~~
1. This should cause an error if <code>GeoPandas</code> is not installed
<code>ModuleNotFoundError: No module named \'geopandas\'</code>
2. The error should be gone</p>
        </div>
    </div>
...
content_html = '''
    ...
    <div class="callout callout-important">
        ...
        <div class="callout-body">
            <p>Il est déconseillé d\'utiliser des importations globales sous la forme <code>from pkg import *</code>. Par exemple, prenons deux modules qui proposent une fonction <code>sqrt</code>:</p>
        </div>
    </div>
...
content_html = '''
    ...
    <div class="callout callout-important">
        ...
        <div class="callout-body">
            <p>It is discouraged to use global imports like <code>from pkg import *</code>. For example, consider two modules that provide a <code>sqrt</code> function:</p>
        </div>
    </div>
